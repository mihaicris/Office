...
function afiseaza_tabel($query, $count)
{
	...
	for ($i = 0; $row = $query->fetch(); $i++) {
		...
		echo '<td id="f' . $row['id_vanzator'] . '"><span class="id">' . $row['id_vanzator'] . '</span><span class="sosa actiune">a</span></td>';
...
if (isset($_POST["creare"])) {
	...
	<form action="/" method="post" id="creare_vanzator">
		<label for="nume_vanzator">Nume</label><br>
		<input class="normal mediu" id="nume_vanzator" type="text" name="nume_vanzator" autocomplete="off"
			   value="<?php echo $_POST["nume"]; ?>"/>
		<img id="c_nume" src="" alt=""><br>
		<label for="prenume_vanzator">Prenume</label><br>
		<input class="normal mediu" id="prenume_vanzator" type="text" name="prenume_vanzator" autocomplete="off" value=""
			   autofocus/>
		<img id="c_prenume" src="" alt=""><br>
...
if (isset($_POST["editeaza"])) {
	...
	<form action="/" method="post">
		<input id="id_vanzator" type="hidden" name="id_vanzator" value="<?php echo $row['id_vanzator']; ?>"/>
		<label>Nume</label><br>
		<input class="normal mediu" id="nume_vanzator" type="text" name="nume_vanzator" autocomplete="off"
			   value="<?php echo $row['nume_vanzator']; ?>"/>
		<img id="c_nume" src="" alt=""><br>
		<label>Prenume</label><br>
		<input class="normal mediu" id="prenume_vanzator" type="text" name="prenume_vanzator" autocomplete="off"
...
if (isset($_POST["sterge"])) {
	$id = $_POST["id"];
	$string = 'DELETE FROM `office`.`vanzatori` WHERE `vanzatori`.`id_vanzator` = ' . $id . ' LIMIT 1;';
